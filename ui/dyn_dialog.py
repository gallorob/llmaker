	...
	def type_changed(self, t: str) -> None:
		...
		if t_enum == EntityEnum.ENEMY:
			...
			if self.level.current_room in self.level.corridors.keys():
				...
				self.roomname_widget.setCurrentText(list(self.level.corridors.keys())[0])
...
class UpdateEntityDialog(UserModeDialog):
	...
	def type_changed(self, t: str) -> None:
		...
		self.ref_name_widget.addItems([x.name for x in self.get_encounter().entities[t]])
	...
	def roomname_changed(self, roomname: str) -> None:
		...
		self.ref_name_widget.clear()
		self.ref_name_widget.addItems([x.name for x in self.get_encounter().entities[self.type_widget.currentText()]])
	...
	def refname_changed(self, refname: str) -> None:
		...
		entity = encounter.get_entity_by_name(entity_type=t_enum,
											  entity_name=refname)
