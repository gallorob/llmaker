...
levels = ["my_levels/space_level", "my_levels/alt_level2"]

logs = ["logs/log_20250513151645.log", "logs/log_20250513151900.log"]
...
def analyze_level(level_name: str) -> None:
    ...
    print(
        f"Average user message length: {avg_user_msg_len:.2f} (min: {min(user_msg_lens)}; max: {max(user_msg_lens)})"
    )
    print(
        f"Average AI message length: {avg_ai_msg_len:.2f} (min: {min(ai_msg_lens)}; max: {max(ai_msg_lens)})"
    )
    ...
    entities_properties = {
        t: {"semantics": [], "n_per_room": [], "n_per_corridor": []}
    ...
    for room in level.rooms.values():
        for t in ["enemy", "treasure", "trap"]:
            ...
            for entity in room.encounter.entities[t]:
                entities_properties[t]["semantics"].append(
    ...
    for t in ["enemy", "treasure", "trap"]:
        print(
            f'Number of {t}: {sum(entities_properties[t]["n_per_room"]) + sum(entities_properties[t]["n_per_corridor"])}'
        )
    room_semantics_str = [
        f'{room_semantic["name"]}: {room_semantic["description"]}'
        for room_semantic in room_semantics
    ]
    ...
    enemy_semantics_str = [
        f'{enemy_semantic["name"]}: {enemy_semantic["description"]}'
        for enemy_semantic in entities_properties["enemy"]["semantics"]
    ]
    ...
    trap_semantics_str = [
        f'{trap_semantic["name"]}: {trap_semantic["description"]}'
        for trap_semantic in entities_properties["trap"]["semantics"]
    ]
    ...
    treasure_semantics_str = [
        f'{treasure_semantic["name"]}: {treasure_semantic["description"]}'
        for treasure_semantic in entities_properties["treasure"]["semantics"]
    ]
...
def analyze_log(log_name: str) -> None:
    ...
    print(
        f"Average FREYR duration: {sum(all_freyr_times) / len(all_freyr_times):.2f}s (min: {min(all_freyr_times):.2f}s; max: {max(all_freyr_times):.2f}s)"
    )
    ...
    print(
        f"Average SD duration: {sum(all_sd_times) / len(all_sd_times):.2f}s (min: {min(all_sd_times):.2f}s; max: {max(all_sd_times):.2f}s)"
    )
    ...
    print(
        f"Time elapsed by FREYR: {freyr_time} ({freyr_time.total_seconds() / session_duration.total_seconds():.2%})"
    )
    ...
    print(
        f"Time elapsed by SD: {sd_time} ({sd_time.total_seconds() / session_duration.total_seconds():.2%})"
    )

    print(
        f"Time elapsed by user: {session_duration - freyr_time - sd_time} ({(session_duration.total_seconds() - freyr_time.total_seconds() - sd_time.total_seconds()) / session_duration.total_seconds():.2%})"
    )
